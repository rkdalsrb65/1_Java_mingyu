...
//		String input = br.readLine(); // == sc.nextLine();
		...
		// 보통 개발자의 실수로 발생하는 예외로 필수적으로 처리하지 않아도 된다.
		// == Unchecked Exception
	...
	public void ex2() {
		...
		try { // try : 예외가 발생할 것 같은 구문을 내부에 작성
	...
	public void ex3() {
		...
		// catch문이 여러 개면 위에서부터 순서대로 발생한 예외를 검사하여 알맞은 매개변수를 가진 catch에서 처리한다
	...
	// It is already handled by the catch block for Exception
			...
			// finally : catch 구문 수행 여부에 관계 없이 무조건 실행하는 구문
		}
	...
	// * 예외 발생 == throw new 예외 클래스();
	//			== 예외 객체가 생성되어 던져짐
	
	// * catch(부모타입 예외 참조 변수) {}
	// -> try에서 던져진 예외 객체가 catch구문 매개변수의 자식 타입이면
	// 부모 타입 예외 참조 변수 = 던져진 자식 객체 -> 다형성(업캐스팅)이 적용되어 해당 catch문에 처리된다.
