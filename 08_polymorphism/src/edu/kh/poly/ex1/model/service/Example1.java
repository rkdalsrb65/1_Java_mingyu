...
//		c2.setWeight(1.5); // 에러
		...
		// 참조변수가 부모 타입이기 때문에 참조하는 객체가 자식이여도 부모 부분만 참조 가능
...
//		System.out.println(c1.toString());
		...
		// 동적 바인딩 : 프로그램 실행 중 참조하는 객체의 실제 타입의 오버라이딩된 메서드 연결
	...
	public void ex2() {
		...
		for(int i = 0; i < arr.length; i++) {
			...
			// arr[i] == 참조변수
			// print() 메서드에 참조변수를 작성하면 자동으로 toString() 메서드 호출
		}
	...
	public void printCar(Car c) {
		...
		// 전달 받은 객체가 자식 객체이고 toString()을 오버라이딩 했다면
...
//		((Truck)c3).loading();
	...
	public void ex4() {
		...
		if(c instanceof Car) {
			// c가 참조하는 객체는 Spark 이ㅣ만
			// 업 캐스팅 상태이기 때문에 Car로도 인식됨
